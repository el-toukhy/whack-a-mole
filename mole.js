...
function popUpRandomMole() {
  ...
  const moleHeads = document.querySelectorAll('.moleHead');
  ...
  moleHead.classList.remove('moleHead--hidden', 'moleHead--whacked');
...
function hideMole(mole) {
  clickable = false;
  mole.classList.add('moleHead--hidden');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  for (let moleHead of moleHeads) {
    moleHead.addEventListener('click', event => {
      ...
      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED
      event.target.classList.add('moleHead--hidden');

      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED FOR THE BONUS
      event.target.classList.add('moleHead--whacked');
    });
  }
});
